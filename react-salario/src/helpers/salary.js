...
const INSS_TABLE = [
  ...
  function calculateSalaryFrom(fullSalary) {
    ...
    const disINSS100 =  ((discountINSS*100)/fullSalary).toFixed(2)
    //disINSS100.toFixed(2);
  
    const baseIRPF = baseINSS - discountINSS;
    const discountIRPF = calculateDiscountIRPF(baseIRPF);
    const disIRPF100 = ((discountIRPF*100)/fullSalary).toFixed(2)
    //disIRPF100.toFixed(2);
  
    const newSalary
     = baseINSS - discountINSS - discountIRPF;
    
     
     
    const newSalary100 = ((newSalary*100)/fullSalary).toFixed(2);
    //newSalary100.toFixed(2);
